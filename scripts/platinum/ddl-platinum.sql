...
CREATE MATERIALIZED VIEW platinum_promotion_effectiveness AS
SELECT 
    s.store_id,
    st.store_type,
    st.store_size,
    ...
    MAX(s.is_holiday) AS has_holiday
    
FROM silver.silver_sales s
LEFT JOIN silver.silver_economic_features f ON s.store_id = f.store_id AND s.date = f.date
LEFT JOIN silver.silver_stores st ON s.store_id = st.store_id AND st.is_current = TRUE
GROUP BY s.store_id, st.store_type, st.store_size, DATE_TRUNC('week', s.date), s.department_id;

CREATE INDEX idx_plat_promo_week ON platinum_promotion_effectiveness (week_start);
CREATE INDEX idx_plat_promo_store ON platinum_promotion_effectiveness (store_id);

-- platinum_sales_trend_analysis
-- Purpose: Pre-aggregate sales trends for faster dashboarding
CREATE MATERIALIZED VIEW platinum_sales_trend_analysis AS
SELECT 
    s.store_id,
    st.store_type,
    s.department_id,
...
CREATE INDEX idx_plat_trend_week ON platinum_sales_trend_analysis (week_start);
CREATE INDEX idx_plat_trend_month ON platinum_sales_trend_analysis (month_start);